...
func (pr *PsqlRecipientRepository) SelectAllRecipient() (recipients []models.Recipient, err error) {
	selRec, err := pr.conn.Query("SELECT id ,firstname, lastname, address,occupation,username,password,phone,email FROM recipient")
	if err != nil {
		return recipients, errors.New("something")
	}
	recp := models.Recipient{}
	for selRec.Next() {
		err := selRec.Scan(&recp.RecipientNo, &recp.FirstName, &recp.LastName, &recp.Address, &recp.Occupation, &recp.Username, &recp.Password, &recp.PhoneNumber, &recp.EmailAddress)
		if err != nil {
...
func (pr *PsqlRecipientRepository) RecipientByUsername(username string) (*models.Recipient, error) {
	...
	err = querystmt.QueryRow(username).Scan(&recipient.FirstName, &recipient.LastName, &recipient.Address, &recipient.Occupation, &recipient.Username, &recipient.Password, &recipient.PhoneNumber, &recipient.EmailAddress, &recipient.RoleID)
	if err != nil {
		fmt.Println("err", err)
		return recipient, err
	}
...
func (pr *PsqlRecipientRepository) UpdateRecipientById(recipient *models.Recipient) error {
	...
	_, err := pr.conn.Exec("UPDATE recipient set firstname = $2, lastname = $3, phone = $4, address = $5, email = $6, occupation = $7, username = $8, password = $9 where id=$1", recipient.RecipientNo, recipient.FirstName, recipient.LastName, recipient.PhoneNumber, recipient.Address, recipient.EmailAddress, recipient.Occupation, recipient.Username, recipient.Password)
