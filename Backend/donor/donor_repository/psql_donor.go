...
func (pr *PsqlDonorRepository) UpdateDonorById(donor *models.Donor) error {
	...
	_, err := pr.conn.Exec("UPDATE donor set firstname = $2, lastname = $3, phone = $4, address = $5, email = $6, occupation = $7, username = $8, password = $9 where id=$1", donor.DonorNo, donor.FirstName, donor.LastName, donor.PhoneNumber, donor.Address, donor.EmailAddress, donor.Occupation, donor.Username, donor.Password)
