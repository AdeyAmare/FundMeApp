...
func (rch *RecipientInfoHandler) CreateRecipientInfo(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		recipientInfo := models.RecipientInfo{
			...
			Recipient:   &recp,
			Date:        time.Now(),
			BankAccount: r.FormValue("accountNo"),
		...
		erro := rch.recpInfoService.CreateRecipientInfo(recipientInfo, recp)
...
// func (rch *RecipientInfoHandler) SelectAll(w http.ResponseWriter, r *http.Request) {
// 	//rNo, _ := strconv.Atoi(r.FormValue("rNo"))

// 	_, err := rch.recpInfoService.SelectAll()
...
func writeImageFile(mf *multipart.File, fname string) {
	...
	mypath := filepath.Join(wd, "web", "assets", "img", fname)
	image, err := os.Create(mypath)
...
func writeFile(mf *multipart.File, fname string) {
	...
	mypath := filepath.Join(wd, "web", "assets", "attachment", fname)
	attachment, err := os.Create(mypath)
