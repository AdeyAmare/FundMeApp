...
import (
	...
	"github.com/FundStation2/admin/admin_repository"
	"github.com/FundStation2/admin/admin_service"
	...
	"github.com/FundStation2/disaster/disaster_repository"
	"github.com/FundStation2/disaster/disaster_service"
	...
	"github.com/FundStation2/role/role_repository"
	"github.com/FundStation2/role/role_service"
...
func main() {
	...
	bankRepo := bank_repository.NewPsqlBankRepository(dbcon)
	...
	dah := REST_API.NewDonorApiHandler(donServ, sessionSrv, roleServ, sess, csrfSignKey)
	rah := REST_API.NewRecipientApiHandler(recServ, sessionSrv, roleServ, sess, csrfSignKey)
	riah := REST_API.NewRecipientInfoApiHandler(recInfoServ)
	cah := REST_API.NewCategoryApiHandler(catServ)
	bah := REST_API.NewBankApiHandler(bankServ)
	aah := REST_API.NewAdminApiHandler(admServ, sessionSrv, roleServ, sess, csrfSignKey)
	...
	http.HandleFunc("/logout", rch.Logout)

	http.HandleFunc("/recipientInfo", rich.CreateRecipientInfo) //Check this later
	...
	http.HandleFunc("/category/education", ch.SelectEducationalCategory)
	...
	http.Handle("category/donation", dch.Authorized(http.HandlerFunc(ch.Donation)))
	...
	//for the REST API
	http.Handle("/v1/donors", aah.Authenticated(ah.Authorized(http.HandlerFunc(dah.GetDonors))))
	http.Handle("/v1/donor/", aah.Authenticated(ah.Authorized(http.HandlerFunc(dah.GetDonor))))
	http.HandleFunc("/v1/donor/login", dah.LoginDonor)
	http.HandleFunc("/v1/donor", dah.PostDonor)
	http.HandleFunc("/v1/donor/update/", dah.PutDonor)
	http.Handle("/v1/donor/delete/", dah.Authenticated(ah.Authorized(http.HandlerFunc(dah.DeleteDonor))))

	http.Handle("/v1/recipients", aah.Authenticated(ah.Authorized(http.HandlerFunc(rah.GetRecipients))))
	http.HandleFunc("/v1/recipient/", rah.GetRecipient)
	http.HandleFunc("/v1/recipient/login", rah.LoginRecipient)
	...
	http.Handle("/v1/recipientsinfo", aah.Authenticated(ah.Authorized(http.HandlerFunc(riah.GetRecipientsInfo))))
	http.Handle("/v1/recipientinfo/", aah.Authenticated(ah.Authorized(http.HandlerFunc(riah.GetRecipientInfo))))
	...
	http.HandleFunc("/v1/categories", cah.GetCategories)
	http.HandleFunc("/v1/category", cah.PostCategory)
	http.HandleFunc("/v1/image", cah.RecieveImage)

	http.HandleFunc("/v1/transfer", bah.Transfer)
	http.Handle("/v1/admin", aah.Authenticated(ah.Authorized(http.HandlerFunc(aah.GetAdmin))))
