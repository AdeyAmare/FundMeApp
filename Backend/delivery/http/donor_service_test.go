...
func TestDonorUpdate(t *testing.T) {
	// tmpl := template.Must(template.ParseGlob("../../web/template"))
	...
	// mux := http.NewServeMux()
	// mux.HandleFunc("/donor/update",donorHandler.UpdateDonor)
	// ts := httptest.NewTLSServer(mux)
	// defer ts.Close()
	...
	// form.Add("id", string(models.DonorMock.DonorNo))
	// form.Add("firstname",string(models.DonorMock.FirstName))
	// form.Add("add", models.DonorMock.Address)
	// form.Add("userName", models.DonorMock.Username)
	// form.Add("password", models.DonorMock.Password)
	// form.Add("phone", models.DonorMock.PhoneNumber)
	...
	// resp, err:= tc.PostForm(sURL+"/donor/update?id=1")
